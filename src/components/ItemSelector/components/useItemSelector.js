...
function nameFilter(input) {
  return item => item.name.toLowerCase().includes(input.toLowerCase())
}

function useItemSelector(formName) {
  ...
  if(value) {
    collectionGroup.results = collectionGroup.results.filter(nameFilter(value))
    wishlistGroup.results = wishlistGroup.results.filter(nameFilter(value))
  }

  return {
    search: value,
    totalResults: collectionGroup.results.length + wishlistGroup.results.length, 
    groups: [collectionGroup, wishlistGroup],
    bgg: bggGroup,
  }
}
